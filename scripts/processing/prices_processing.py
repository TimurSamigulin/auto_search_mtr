"""
    В данном файле содержится класс по парсингу котировок меди и аллюминия, а также курса доллара к рублю. Также
    производится расчет средних показателей по годам и месяцам и расчет базовый цены кабеля с медной и алюминевой жилой.

    Функции:
    parse_share_price -> pandas.DataFrame, функция парсит котировки металлов и валют
    mean_price -> pandas.DataFrame, расчет средней цены по годам
    mean_price_month -> pandas.DataFrame, расчет средней цены по месяцам
    base_price -> int, расчет базовой цены кабеля
"""
