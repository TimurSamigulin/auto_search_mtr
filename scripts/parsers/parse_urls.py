"""
    Файл содержащий класс SiteSearch для поиска источников в интернете
    Функции:
    write_urls_in_file, сохраняет список переданных url в файл
    get_urls -> list, парсит поисковые выдачи из Яндекса и Google по переданному поисковому запросу
    is_shop -> bool, проверяет является ли переданный сайт интернет-магазином или нет
    all_urls -> list, парсит все ссылки из поисковой выдачи по переданным поисковым запросам
    get_model_query -> list, загружает поисковые запросы из файла
    get_all_urls -> list, парсит все ссылки из поисковой выдачи по переданным поисковым запросам и сохраняет в файл
    check_shop_url -> list проверяет переданные url сайтов, являются ли они магазинами
"""
